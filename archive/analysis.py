...
def get_nellie_inputs(raw_basename, nellie_outputs, nellie_output_files):
    ...
    print(nellie_feature_branch_files)
...
for im in range(len(all_im)):
    temp_labeled_im = labeled_im[im]
    binary_im = temp_labeled_im .astype(bool)
    unique_label = np.unique(temp_labeled_im )
    print(len(unique_label))

    num_features = len(unique_label) -1 if 0 in unique_label else len(unique_label)

    print(f"feature in {im} frame is {num_features}")


    #calculate centriod to label number feature
    centriod = center_of_mass(temp_labeled_im ,temp_labeled_im , range(1, num_features+1))
    all_text[im] = np.array(centriod)
    all_string[im] = [str(i+1) for i in range(num_features)]

print(all_string.shape)
...
def get_nellie_outputs(csv):
    ...
    #initilizing maximum frame number + 1 since itstart from 0  ??? 
    ...
    #iterate through all frame
    for t in range(max_frame_num):
        #.loc access a group of row and column by label  
        # all label in frame t
        num_unique_labels_in_t = len(nellie_df_small.loc[nellie_df_small['t'] == t, 'label'].unique())

        # number of label in each frame - number of all reassigned label?????
        ...
        if label_differences is None:
            ...
            continue
        #subtract label diff of the current frame from the latest frame
    ...
    return fission_events, fusion_events, events_per_frame
